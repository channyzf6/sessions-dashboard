...
//   1. We invoke Playwright's CLI directly via require.resolve instead
//      of `npx playwright install`. Playwright restricts subpath
//      imports via package.json `exports`, so resolving cli.js
//      directly throws ERR_PACKAGE_PATH_NOT_EXPORTED — but package.json
//      itself is always exported, so resolve that and walk to cli.js.
//      Avoids the npx layer entirely.
//   2. We spawn with cwd set to sessions-dashboard's own package root
//      (where playwright IS a direct dep) rather than inheriting the
//      caller's cwd. Without this, Playwright's "running outside
//      project context" check looks at the user's invocation cwd
//      (e.g. wherever they typed `npx -y sessions-dashboard install`)
//      and fires a misleading warning even though Playwright is
//      installed correctly inside our package's node_modules.
export function prefetchChromium(log = console.log) {
  return new Promise((resolve, reject) => {
    ...
    try {
      ...
      // Walk lib/installer.mjs -> lib -> sessions-dashboard package root.
      // That root has package.json with playwright as a direct dep, which
      // is what Playwright's outside-project check is looking for.
    ...
    const child = spawn(
      ...
      { stdio: "inherit", windowsHide: true, cwd: pkgRoot },
    );
